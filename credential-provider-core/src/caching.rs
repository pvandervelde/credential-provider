...
mod tests {
    ...
    use crate::{
        CachingCredentialProvider, CredentialError, CredentialProvider, HmacSecret,
        UsernamePassword,
    };
    ...
    async fn empty_cache_result_is_cached_second_call_does_not_refetch() {
        let mock = MockCredentialProvider::from_sequence(vec![
            Ok(valid_up("alice")),
            Err(CredentialError::Backend("inner must not be called a second time".into())),
        ]);
        let caching = CachingCredentialProvider::new(mock, REFRESH_WINDOW);
        ...
        assert!(second.is_ok(), "second call must return cached credential, not an error");
    ...
    async fn empty_cache_inner_failure_returns_unavailable() {
        let mock = MockCredentialProvider::<UsernamePassword>::returning_err(
            CredentialError::Backend("vault unreachable".into()),
        );
        let caching = CachingCredentialProvider::new(mock, REFRESH_WINDOW);
    ...
    async fn valid_credential_outside_refresh_window_not_fetched_again() {
        let mock = MockCredentialProvider::from_sequence(vec![
            Ok(valid_up("alice")),
            Err(CredentialError::Backend("inner must not be called on cache hit".into())),
        ]);
        let caching = CachingCredentialProvider::new(mock, REFRESH_WINDOW);
    ...
    async fn no_expiry_credential_inner_called_only_once_across_many_calls() {
        let mock = MockCredentialProvider::from_sequence(vec![
            ...
            Err(CredentialError::Backend("inner must not be called again".into())),
        ]);
        let caching = CachingCredentialProvider::new(mock, REFRESH_WINDOW);
    ...
    async fn after_successful_refresh_subsequent_call_returns_new_credential() {
        ...
        assert_eq!(
            third.unwrap().username, "new-user",
