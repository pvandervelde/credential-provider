...
| `extract_str_field` | `fn` | `credential-provider::vault` | Reads a named string field from a Vault response `data` JSON object. Returns `CredentialError::Backend("missing field: {name}")` when the field is absent or not a JSON string. Use in every `VaultExtractor` impl within `credential-provider/src/vault.rs`. For extractors outside this module, replicate the pattern inline. | vault, extractor, json, error-handling |
